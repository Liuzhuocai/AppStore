package com.elf.appstore.adapter.base;

import android.support.v7.widget.RecyclerView;
import android.util.SparseArray;
import android.view.View;

import com.elf.appstore.model.CategoryItem;
import com.elf.appstore.model.NetAppItem;
import com.elf.appstore.model.entities.BannerInfo;

/**
 * Created by antino on 18-3-31.
 */

public abstract class CommonViewHolder<T> extends RecyclerView.ViewHolder{
    // SparseArray 比 HashMap 更省内存，在某些条件下性能更好，只能存储 key 为 int 类型的数据，
    // 用来存放 View 以减少 findViewById 的次数
    private SparseArray<View> viewSparseArray;
    protected OnItemCommonClickListener commonClickListener;

    public CommonViewHolder(View itemView) {
        super(itemView);
        viewSparseArray = new SparseArray<>();
    }

    /**
     * 根据 ID 来获取 View *
     * @param viewId viewID
     * @param <T> 泛型
     * @return 将结果强转为 View 或 View 的子类型
     */
    public <T extends View> T getView(int viewId) {
        // 先从缓存中找，找打的话则直接返回
        // 如果找不到则 findViewById ，再把结果存入缓存中
        View view = viewSparseArray.get(viewId);
        if (view == null) {
            view = itemView.findViewById(viewId);
            viewSparseArray.put(viewId, view);
        }
        return (T) view;
    }

    public interface OnItemCommonClickListener {
        void onBannerClickListener(BannerInfo info);
        void onMoreClickListener(CategoryItem info);
        void onDownloadClick(NetAppItem info);
        void onItemClickListener(NetAppItem position);
        void onItemLongClickListener(int position);
    }

    public void setCommonClickListener(OnItemCommonClickListener commonClickListener) {
        this.commonClickListener = commonClickListener;
    }

    public abstract void bindData(T data);
}
